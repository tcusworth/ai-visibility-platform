...
export function buildVisibilityScorerPrompt(input: VisibilityScorerPromptInput): string {
  ...
  return [
    ...
    `4. Do not classify ${p.targetReferences.join(", ")} as an external competitor entity.`,
    ...
    "CITATION RULE",
    "",
    `Set ${p.citedField} to true only when the supplied sources contain a ${p.targetShortName}-controlled source, or when the answer explicitly attributes supporting evidence to ${p.targetShortName}.`,
    "",
    "PLATFORM:",
    input.platform,
    "",
