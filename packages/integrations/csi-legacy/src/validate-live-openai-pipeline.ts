...
class TwoRequestHttpClient implements HttpJsonClient {
  ...
  async postJson(url: string, headers: Record<string, string>, body: unknown): Promise<HttpJsonResponse> {
    this.calls += 1;
    if (this.calls > 2) throw new Error("Live OpenAI pipeline attempted more than two HTTP requests.");
...
function requireLocalDb(value: string): void {
  ...
  if (!["localhost", "127.0.0.1", "::1"].includes(url.hostname) || url.pathname.replace(/^\//, "") !== "ai_visibility_dev") {
    throw new Error("Live OpenAI pipeline validation is restricted to local ai_visibility_dev.");
  }
}
...
async function main(): Promise<void> {
  if (process.env.ALLOW_LIVE_OPENAI_PIPELINE !== "YES") {
    throw new Error("Set ALLOW_LIVE_OPENAI_PIPELINE=YES to authorize exactly two OpenAI requests.");
  }
  ...
  const benchmarkModel = process.env.OPENAI_BENCHMARK_MODEL?.trim() || "gpt-5.6-luna";
  const scorerModel = process.env.SCORER_MODEL?.trim() || "gpt-5.4-nano";
  const pool = new Pool({ connectionString, ssl: false, max: 4 });
  const runId = randomUUID();
  const runKey = `live-openai-pipeline-validation-${randomUUID()}`;
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO benchmark_runs (
       ...
       ) VALUES ($1,$2,$3,$4,CURRENT_DATE,'running',1,1,1,0,0,false,'live-openai-pipeline-validation-v1')`,
      [runId, workspaceId, def.id, runKey]);

    const providerPrompt = `You are answering a question specifically about industrial Open Process Automation (OPA) and the O-PAS™ Standard from The Open Group.
...
${prompt.text}`;
    ...
    const provider = new OpenAIResponsesProvider({ apiKey, httpClient, webSearch: true });
    const scorer = new OpenAIResponsesScorerClient({ apiKey, model: scorerModel, httpClient });
    ...
    console.log(`Benchmark provider model requested: ${benchmarkModel}`);
    console.log(`Scorer model requested: ${scorerModel}`);
    console.log("Making exactly two OpenAI requests: one benchmark-provider request and one scorer request...");
    ...
    assert(httpClient.calls === 2, "Exactly two OpenAI HTTP requests were made");
    ...
    console.log(`Benchmark provider response ID present: ${result.providerRequestId ? "yes" : "no"}`);
    console.log(`Benchmark provider model returned: ${result.scored.observation.model}`);
    console.log(`Scorer model returned: ${result.scored.scorerModel}`);
    ...
    console.log("Live OpenAI end-to-end observation validation passed.");
    console.log("Exactly one benchmark-provider request, one scorer request, and temporary local database writes were made.");
    console.log("No n8n, Google Sheets, Gemini, Claude, Perplexity, or CSI production calls were made.");
  } finally {
