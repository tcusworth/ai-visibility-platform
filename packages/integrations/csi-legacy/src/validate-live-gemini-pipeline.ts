...
class TwoRequestHttpClient implements HttpJsonClient {
  ...
  async postJson(url: string, headers: Record<string, string>, body: unknown): Promise<HttpJsonResponse> {
    this.calls += 1;
    if (this.calls > 2) throw new Error("Live Gemini pipeline attempted more than two HTTP requests.");
...
function requireLocalDb(value: string): void {
  ...
  if (!["localhost", "127.0.0.1", "::1"].includes(url.hostname) || url.pathname.replace(/^\//, "") !== "ai_visibility_dev") {
    throw new Error("Live Gemini pipeline validation is restricted to local ai_visibility_dev.");
  }
}
...
async function main(): Promise<void> {
  if (process.env.ALLOW_LIVE_GEMINI_PIPELINE !== "YES") {
    throw new Error("Set ALLOW_LIVE_GEMINI_PIPELINE=YES to authorize one Gemini provider request and one OpenAI scorer request.");
  }
  ...
  const benchmarkModel = process.env.GEMINI_BENCHMARK_MODEL?.trim() || "gemini-3.7-flash";
  const scorerModel = process.env.SCORER_MODEL?.trim() || "gpt-5.4-nano";
  const pool = new Pool({ connectionString, ssl: false, max: 4 });
  const runId = randomUUID();
  const runKey = `live-gemini-pipeline-validation-${randomUUID()}`;
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO benchmark_runs (
       ...
       ) VALUES ($1,$2,$3,$4,CURRENT_DATE,'running',1,1,1,0,0,false,'live-gemini-pipeline-validation-v1')`,
...
${prompt.text}`;
    ...
    const provider = new GeminiInteractionsProvider({ apiKey: geminiApiKey, httpClient });
    ...
    console.log(`Gemini benchmark provider model requested: ${benchmarkModel}`);
    console.log(`OpenAI scorer model requested: ${scorerModel}`);
    console.log("Making exactly two HTTP requests: one Gemini benchmark-provider request and one OpenAI scorer request...");
    ...
    console.log(`Gemini provider response ID present: ${result.providerRequestId ? "yes" : "no"}`);
    console.log(`Gemini provider model returned: ${result.scored.observation.model}`);
    ...
    console.log("Live Gemini end-to-end observation validation passed.");
    console.log("Exactly one Gemini benchmark-provider request, one OpenAI scorer request, and temporary local database writes were made.");
    console.log("No n8n, Google Sheets, Claude, Perplexity, or CSI production calls were made.");
  } finally {
