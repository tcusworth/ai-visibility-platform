...
class CountingFetchHttpJsonClient implements HttpJsonClient {
  ...
  async postJson(url: string, headers: Record<string, string>, body: unknown): Promise<HttpJsonResponse> {
    this.calls += 1;
    if (this.calls > 1) throw new Error("Live Claude provider validator attempted more than one HTTP request.");
...
if (process.env.ALLOW_LIVE_CLAUDE_PROVIDER !== "YES") {
  throw new Error(
    "Live Claude benchmark-provider call blocked. Set ALLOW_LIVE_CLAUDE_PROVIDER=YES to authorize exactly one request.",
  );
}

const apiKey = requireEnv("ANTHROPIC_API_KEY");
const model = process.env.CLAUDE_BENCHMARK_MODEL?.trim() || "claude-sonnet-4-6";
const httpClient = new CountingFetchHttpJsonClient();
const provider = new ClaudeMessagesProvider({ apiKey, httpClient });
...
console.log(`Claude benchmark provider model requested: ${model}`);
console.log("Making exactly one isolated Claude benchmark-provider request with web search enabled...");
...
if (httpClient.calls !== 1) {
  throw new Error(`Expected exactly one Claude provider HTTP request, observed ${httpClient.calls}.`);
}
if (!response.answer.trim()) throw new Error("Claude provider returned an empty answer.");
if (!response.model.trim()) throw new Error("Claude provider returned an empty model identifier.");

const sources = response.sources ?? [];
const uniqueDomains = [...new Set(sources.map((source) => source.domain))].sort();

console.log(`Claude benchmark provider model returned: ${response.model}`);
...
console.log("Isolated Claude benchmark-provider validation completed.");
console.log("Exactly one Claude benchmark-provider request was made.");
