...
class CountingFetchHttpJsonClient implements HttpJsonClient {
  ...
  async postJson(url: string, headers: Record<string, string>, body: unknown): Promise<HttpJsonResponse> {
    this.calls += 1;
    if (this.calls > 1) throw new Error("Live Gemini provider validator attempted more than one HTTP request.");
...
if (process.env.ALLOW_LIVE_GEMINI_PROVIDER !== "YES") {
  throw new Error(
    "Live Gemini benchmark-provider call blocked. Set ALLOW_LIVE_GEMINI_PROVIDER=YES to authorize exactly one request.",
  );
}

const apiKey = requireEnv("GEMINI_API_KEY");
const model = process.env.GEMINI_BENCHMARK_MODEL?.trim() || "gemini-3.7-flash";
...
Use Google Search when useful to identify relevant companies, organizations, integrators, suppliers, standards sources, technical resources, and practitioners.
...
console.log(`Gemini benchmark provider model requested: ${model}`);
console.log("Making exactly one isolated Gemini benchmark-provider request with Google Search enabled...");
...
if (httpClient.calls !== 1) {
  throw new Error(`Expected exactly one Gemini provider HTTP request, observed ${httpClient.calls}.`);
}
if (!response.answer.trim()) throw new Error("Gemini provider returned an empty answer.");
if (!response.model.trim()) throw new Error("Gemini provider returned an empty model identifier.");

const sources = response.sources ?? [];
const uniqueDomains = [...new Set(sources.map((source) => source.domain))].sort();

console.log(`Gemini benchmark provider model returned: ${response.model}`);
...
console.log("Isolated Gemini benchmark-provider validation completed.");
console.log("Exactly one Gemini benchmark-provider request was made.");
