...
class TwoRequestHttpClient implements HttpJsonClient {
  ...
  async postJson(url: string, headers: Record<string, string>, body: unknown): Promise<HttpJsonResponse> {
    this.calls += 1;
    if (this.calls > 2) throw new Error("Live Claude pipeline attempted more than two HTTP requests.");
...
function requireLocalDb(value: string): void {
  ...
  if (!["localhost", "127.0.0.1", "::1"].includes(url.hostname) || url.pathname.replace(/^\//, "") !== "ai_visibility_dev") {
    throw new Error("Live Claude pipeline validation is restricted to local ai_visibility_dev.");
  }
}
...
async function main(): Promise<void> {
  if (process.env.ALLOW_LIVE_CLAUDE_PIPELINE !== "YES") {
    throw new Error("Set ALLOW_LIVE_CLAUDE_PIPELINE=YES to authorize one Claude provider request and one OpenAI scorer request.");
  }
  ...
  const benchmarkModel = process.env.CLAUDE_BENCHMARK_MODEL?.trim() || "claude-sonnet-4-6";
  const scorerModel = process.env.SCORER_MODEL?.trim() || "gpt-5.4-nano";
  const pool = new Pool({ connectionString, ssl: false, max: 4 });
  const runId = randomUUID();
  const runKey = `live-claude-pipeline-validation-${randomUUID()}`;
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO benchmark_runs (
       ...
       ) VALUES ($1,$2,$3,$4,CURRENT_DATE,'running',1,1,1,0,0,false,'live-claude-pipeline-validation-v1')`,
...
${prompt.text}`;
    ...
    const provider = new ClaudeMessagesProvider({ apiKey: anthropicApiKey, httpClient });
    ...
    console.log(`Claude benchmark provider model requested: ${benchmarkModel}`);
    console.log(`OpenAI scorer model requested: ${scorerModel}`);
    console.log("Making exactly two HTTP requests: one Claude benchmark-provider request and one OpenAI scorer request...");
    ...
    console.log(`Claude provider response ID present: ${result.providerRequestId ? "yes" : "no"}`);
    console.log(`Claude provider model returned: ${result.scored.observation.model}`);
    ...
    console.log("Live Claude end-to-end observation validation passed.");
    console.log("Exactly one Claude benchmark-provider request, one OpenAI scorer request, and temporary local database writes were made.");
    console.log("No n8n, Google Sheets, Gemini, Perplexity, or CSI production calls were made.");
  } finally {
